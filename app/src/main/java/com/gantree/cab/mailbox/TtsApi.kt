...
/**
 * `POST /api/tts { text }` → MP3 bytes. The same Worker route the PWA plays
 * through; the phone sends its Bearer session instead of the cookie. 404 until
 * the Worker has a Chirp key (or `VOICE=off`), 401 when the session is gone,
 * 429 too many, 502 when Google refused. Nothing is stored and the Worker never
 * logs the text (pendant docs/voice.md).
 */
class TtsApi(
  ...
  fun synthesize(origin: String, bearer: String, text: String): TtsResult {
